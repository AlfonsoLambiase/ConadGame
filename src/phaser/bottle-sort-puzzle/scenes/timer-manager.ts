...
export class TimerManager extends Phaser.Scene {
  ...
  constructor() {
    super({key: BottleSortPuzzleAssetConf.scene.timerManager});
  }

  create(): void {
    this.addTimer();
    this.registry.set(BottleSortPuzzleAssetConf.registry.timer, this.timerGame);
  }
  ...
  addTimer(): void {
    ...
    const iconTimer = this.add.sprite(
      ...
      BottleSortPuzzleAssetConf.image.iconSandClock,
    );
  ...
  updateTimer(): void {
    ...
    const leftTime = +this.registry.get(BottleSortPuzzleAssetConf.registry.timer) + 1;

    this.registry.set(BottleSortPuzzleAssetConf.registry.timer, leftTime);
    if (this.timerText) {
  ...
  resetTimer(): void {
    this.timer = this.timerGame;
    this.timerTemp = 0;
    this.registry.set(BottleSortPuzzleAssetConf.registry.timer, this.timer);
