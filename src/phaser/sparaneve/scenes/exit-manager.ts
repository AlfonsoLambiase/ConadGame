...
export class ExitManager extends Phaser.Scene {
  ...
  create() {
    ...
    if (this.scene.isActive(assetConf.scene.game)) {
      ...
      GlobalEvents.emit(GameEvents.PAUSE_GAME);
    }
  ...
  #addPopup() {
    ...
    btnCancel.on("pointerdown", () => {
      ...
      if (!this.scene.isActive(assetConf.scene.game)) {
        ...
        // Emetti evento di resume
        console.log("RESUME_GAME emitted");
        GlobalEvents.emit(GameEvents.RESUME_GAME);
      }
    });
  ...
  public createExitButton(scene: Phaser.Scene, theme?: Phaser.Sound.BaseSound) {
    ...
    const exitButton = scene.add
      .image(
        width - this.gameScene.setDynamicValueBasedOnScale(50, 150),
        this.gameScene.setDynamicValueBasedOnScale(50, 150),
      ...
      .setScrollFactor(0)
      .setDepth(100)
      .setScale(this.gameScene.setDynamicValueBasedOnScale(0.5, 1.5));
