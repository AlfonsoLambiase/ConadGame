...
export class Tutorial extends Phaser.Scene {
  ...
  constructor() {
    super({key: assetConf.scene.tutorial});
  }
  ...
  //     this.width, // Ridimensionamento X
  //     1080, // Ridimensionamento Y
  //     //assetConf.image.roadBackground,
  //     "roadBackground",
  //   );
  ...
  addAnims() {
    ...
    this.tutorialPlayer01 = this.#createTutorialPlayerAnimation(
      assetConf.keyAnim.animTutorialPlayer01,
      assetConf.spritesheet.tutorialPlayer01.key,
      38,
    );

    this.tutorialText01 = this.#createTutorialTextAnimation(
      assetConf.keyAnim.animStartTutorialText01,
      assetConf.spritesheet.tutorialText01.key,
      0,
      2,
    );

    this.tutorialText01 = this.#createTutorialTextAnimation(
      assetConf.keyAnim.animEndTutorialText01,
      assetConf.spritesheet.tutorialText01.key,
      3,
      5,
    );
    ...
    this.tutorialPlayer02 = this.#createTutorialPlayerAnimation(
      assetConf.keyAnim.animTutorialPlayer02,
      assetConf.spritesheet.tutorialPlayer02.key,
      37,
    );

    this.tutorialText02 = this.#createTutorialTextAnimation(
      assetConf.keyAnim.animStartTutorialText02,
      assetConf.spritesheet.tutorialText02.key,
      0,
      2,
    );

    this.tutorialText02 = this.#createTutorialTextAnimation(
      assetConf.keyAnim.animEndTutorialText02,
      assetConf.spritesheet.tutorialText02.key,
      3,
      5,
    );
    ...
    this.tutorialPlayer03 = this.#createTutorialPlayerAnimation(
      assetConf.keyAnim.animTutorialPlayer03,
      assetConf.spritesheet.tutorialPlayer03.key,
      38,
    );

    this.tutorialText03 = this.#createTutorialTextAnimation(
      assetConf.keyAnim.animStartTutorialText03,
      assetConf.spritesheet.tutorialText03.key,
      0,
      2,
    );

    this.tutorialText03 = this.#createTutorialTextAnimation(
      assetConf.keyAnim.animEndTutorialText03,
      assetConf.spritesheet.tutorialText03.key,
      3,
      5,
    );
  ...
  addButton(normalSizeButton: number, reduceSizeButton: number) {
    this.startButton = this.add
      .sprite(
        this.width / 2,
        this.setDynamicValueBasedOnScale(this.height - 50, this.height - 450),
        assetConf.image.tutorialTastoAvanti,
      )
      .setInteractive()
      .setOrigin(0.5, 1)
      .setScrollFactor(0)
  ...
  #playTutorial01(normalSizeButton: number, reduceSizeButton: number) {
    ...
    this.tutorialPlayer01.play(assetConf.keyAnim.animTutorialPlayer01);

    this.tutorialPlayer01.on(
      Phaser.Animations.Events.ANIMATION_UPDATE,
      (animation: Phaser.Animations.Animation, frame: Phaser.Animations.AnimationFrame) => {
        if (!this.isTutorialActive) return;
        if (animation.key === assetConf.keyAnim.animTutorialPlayer01 && frame.index === 12) {
          this.tutorialText01.setVisible(true);
          this.tutorialText01.play(assetConf.keyAnim.animStartTutorialText01);
        }
      },
    );

    this.tutorialPlayer01.once(
      Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + assetConf.keyAnim.animTutorialPlayer01,
      () => {
        this.tutorialText01.play(assetConf.keyAnim.animEndTutorialText01);
        this.tutorialText01.once(
          Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + assetConf.keyAnim.animEndTutorialText01,
          () => {
  ...
  #playTutorial02(normalSizeButton: number, reduceSizeButton: number) {
    ...
    this.tutorialPlayer02.play(assetConf.keyAnim.animTutorialPlayer02);

    this.tutorialPlayer02.on(
      Phaser.Animations.Events.ANIMATION_UPDATE,
      (animation: Phaser.Animations.Animation, frame: Phaser.Animations.AnimationFrame) => {
        if (!this.isTutorialActive) return;
        if (animation.key === assetConf.keyAnim.animTutorialPlayer02 && frame.index === 12) {
          this.tutorialText02.setVisible(true);
          this.tutorialText02.play(assetConf.keyAnim.animStartTutorialText02);
        }
      },
    );

    this.tutorialPlayer02.once(
      Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + assetConf.keyAnim.animTutorialPlayer02,
      () => {
        this.tutorialText02.play(assetConf.keyAnim.animEndTutorialText02);
        this.tutorialText02.once(
          Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + assetConf.keyAnim.animEndTutorialText02,
          () => {
  ...
  #playTutorial03(normalSizeButton: number, reduceSizeButton: number) {
    ...
    this.tutorialPlayer03.play(assetConf.keyAnim.animTutorialPlayer03);

    this.tutorialPlayer03.on(
      Phaser.Animations.Events.ANIMATION_UPDATE,
      (animation: Phaser.Animations.Animation, frame: Phaser.Animations.AnimationFrame) => {
        if (!this.isTutorialActive) return;
        if (animation.key === assetConf.keyAnim.animTutorialPlayer03 && frame.index === 12) {
          this.tutorialText03.setVisible(true);
          this.tutorialText03.play(assetConf.keyAnim.animStartTutorialText03);
        }
      },
    );

    this.tutorialPlayer03.once(
      Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + assetConf.keyAnim.animTutorialPlayer03,
      () => {
        this.tutorialText03.play(assetConf.keyAnim.animEndTutorialText03);
        this.tutorialText03.once(
          Phaser.Animations.Events.ANIMATION_COMPLETE_KEY + assetConf.keyAnim.animEndTutorialText03,
          () => {
  ...
  #createTutorialPlayerAnimation(animKey: string, spriteKey: string, frameEnd: number) {
    ...
    const tutorialPlayer = this.add
      .sprite(this.width / 2, this.height / 2, spriteKey)
      .setOrigin(0.5)
      .setVisible(false)
  ...
  #createTutorialTextAnimation(
    ...
    const sprite = this.add
      .sprite(this.width / 2 + 50, this.setDynamicValueBasedOnScale(50, 450), spriteKey)
      .setOrigin(0.5, 0)
      .setVisible(false)
