...
import {GameSparaNeveManager} from "./sparaneve-manager";
...
export class Game extends Phaser.Scene {
  sparaNeveManager!: GameSparaNeveManager;
  mainMenu!: MainMenu;
  ...
  public isGameOver: boolean = false; // controllo aggiuntivo per eveitare che possa andare 2 volte in gameOver
  starsEffectManager!: StarsEffectManager;
  isDownPressed: boolean = false;
  isUpPressed: boolean = false;
  public isShootPressed: boolean = false;
  ...
  init(data: {typeImage: number}) {
    //! Metodo per assegnare direttamente riferimento alla scena Game con registry 1/2
  ...
  private initializeBubbleShooterManager() {
    ...
    if (!this.scene.manager.getScene(assetConf.scene.mainMenu)) {
      this.scene.manager.add(assetConf.scene.mainMenu, MainMenu, false);
    }

    // Avvia la scena BubbleShooterManager
    this.scene.launch(assetConf.scene.mainMenu, {
      gameScene: this,
    });

    // Ottieni riferimento al manager
    this.mainMenu = this.scene.get(assetConf.scene.mainMenu) as MainMenu;
  }
  ...
  gameOver(): void {
    if (!this.isGameOver) {
      ...
      this.scene.stop(assetConf.scene.mobileControlsScene);

      // Emetti evento di pausa
      // console.log("PAUSE_GAME emitted");
      // GlobalEvents.emit(GameEvents.PAUSE_GAME); //! se attivo l evento da errore.
