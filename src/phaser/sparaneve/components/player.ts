...
export default class Player extends Phaser.Physics.Arcade.Sprite {
  ...
  private canThrow(): boolean {
    return this.currentTrack.playerSnowballs.getFirstDead(false) !== null;
  }

  public attemptThrow(): void {
    if (!this.isThrowing && this.canThrow()) {
  ...
  throw(): void {
    // Questo metodo ora chiama semplicemente attemptThrow
    this.attemptThrow();
  }

  releaseSnowball(): void {
    // Prova a lanciare la snowball
    const thrown = this.currentTrack.throwPlayerSnowball(this.x, this);

    if (thrown) {
      // Se il lancio è riuscito, continua con l'animazione
      this.play("throwEnd");
    } else {
      // Se non ci sono snowball disponibili, torna subito a idle
      this.throwComplete();
    }
  }
  throwComplete(): void {
