...
export class Game extends Phaser.Scene {
  ...
  private initializeBubbleShooterManager() {
    ...
    if (!this.scene.manager.getScene(assetConf.scene.tetrisManager)) {
      this.scene.manager.add(assetConf.scene.tetrisManager, TetrisManager, false);
    }

    // Avvia la scena BubbleShooterManager
    this.scene.launch(assetConf.scene.tetrisManager, {
      gameScene: this,
    });

    // Ottieni riferimento al manager
    this.tetrisManager = this.scene.get(assetConf.scene.tetrisManager) as TetrisManager;
  }
