...
import {RaccogliNoteManager} from "./raccogli-note-manager";

const assetConf = RaccogliNoteAssetConf; //* Generalizzazione

export class Game extends Phaser.Scene {
  raccogliNoteManager!: RaccogliNoteManager;
  ...
  init(data: {typeImage: number}) {
    ...
    console.log("GameScene Raccogli note: ", this.typeImage);
  }
  ...
  private initializeGameManager() {
    ...
    if (!this.scene.manager.getScene(assetConf.scene.raccogliNoteManager)) {
      this.scene.manager.add(assetConf.scene.raccogliNoteManager, this.raccogliNoteManager, false);
    }

    // Avvia la scena BubbleShooterManager
    this.scene.launch(assetConf.scene.raccogliNoteManager, {
      gameScene: this,
    });

    // Ottieni riferimento al manager
    this.raccogliNoteManager = this.scene.get(
      assetConf.scene.raccogliNoteManager,
    ) as RaccogliNoteManager;

    // Porta la scena corrente (Game) sopra quella del manager
    this.scene.bringToTop(assetConf.scene.game);
  }
  ...
  gameOver(): void {
    if (!this.isGameOver) {
      ...
      this.raccogliNoteManager.isGameOver = true;
      this.raccogliNoteManager.canShoot = false;
