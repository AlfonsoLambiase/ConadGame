...
export class UIManager {
  ...
  #createBackgroundLogoAndLogo() {
    ...
    if (sponsorLogo !== "empty") {
      ...
      bgLogo.setOrigin(0.5, 0).setScale(this.gameScene.setDynamicValueBasedOnScale(0.4, 1.0)); // centro in alto

      // logo
      const logo = this.scene.add.image(0, 0, "logo");

      logo.setOrigin(0.5, 0.5).setScale(this.gameScene.setDynamicValueBasedOnScale(0.4, 1.0)); // centro pieno

      // Posiziona il logo al centro del bgLogo
      logo.y = bgLogo.displayHeight / 2;
  ...
  #createContainerScore() {
    // 1. Calcolo posizione schermo - ALTO SINISTRA (opposto al TimerManager)
    const safeTop = this.scene.registry.get("safeTop") || 0; //! notch Area
    ...
    const centerY = this.gameScene.setDynamicValueBasedOnScale(180, 380) + this.ofssetY + safeTop; //* Modificare solo questo
