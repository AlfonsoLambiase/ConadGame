...
export class UIManager {
  ...
  playerScoreContainer!: Phaser.GameObjects.Container;
  enemyScoreContainer!: Phaser.GameObjects.Container;
  /** Presente solo se cՏ sponsor logo; usato per allineare elementi sotto la fascia logo */
  logoContainer: Phaser.GameObjects.Container | null = null;
  ...
  #createRpsScoreContainers() {
    const safeTop = this.scene.registry.get("safeTop") || 0;
    const containerScale = this.gameScene.setDynamicValueBasedOnScale(0.4, 0.8);
    const baseY = this.gameScene.setDynamicValueBasedOnScale(170, 330) + safeTop;
    const screenW = this.scene.scale.width;
    const offsetX = this.gameScene.setDynamicValueBasedOnScale(80, 200);

    const buildContainer = (x: number, color: string, iconOnRight: boolean) => {
      ...
      const icon = this.scene.add
        .image(0, 0, assetConf.image.iconScore)
        .setOrigin(0.5)
        .setScale(0.8);
      ...
      const children = iconOnRight ? [bg, text, icon] : [bg, icon, text];
      const container = this.scene.add.container(x, baseY, children);
      container.setScrollFactor(0).setDepth(10).setScale(containerScale);

      return {container, text};
    };

    const player = buildContainer(offsetX, "#cc0000", false);
    this.playerScoreContainer = player.container;
    this.playerScoreText = player.text;

    const enemy = buildContainer(screenW - offsetX, "#0055cc", true);
    this.enemyScoreContainer = enemy.container;
    this.enemyScoreText = enemy.text;
  }

  addPlayerPoint(): void {
    this.playerScore++;
    this.#animateScoreText(this.playerScoreText, this.playerScore);
  }

  addEnemyPoint(): void {
    this.enemyScore++;
    this.#animateScoreText(this.enemyScoreText, this.enemyScore);
  }

  #animateScoreText(text: Phaser.GameObjects.Text, value: number): void {
    text.setText(`${value}`);
    text.setScale(1.3);
