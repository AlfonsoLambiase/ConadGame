...
export class PhotoResultOverlay {
  ...
  show(options: PhotoResultOverlayOptions): void {
    ...
    cornice.on("pointerdown", dismiss);
    ...
    revealContainer.on("pointerdown", dismiss);
    this.photoComposite.setInteractive(
      new Phaser.Geom.Rectangle(0, 0, captureW, captureH),
      Phaser.Geom.Rectangle.Contains,
    );
    this.photoComposite.on("pointerdown", dismiss);

    this.dismissTimer = scene.time.delayedCall(cfg.PHOTO_RESULT_MAX_MS, dismiss);
  }
