...
export class Game extends Phaser.Scene {
  ...
  create() {
    ...
    // UI nella scene Game (overlay fisso); GameManager va sotto per sky/terrain e gameplay.
    ...
    this.audioManager.playLoop("jeepSound");
    ...
    // Game sopra GameManager (HUD visibile); exit sopra tutto.
  ...
  gameOver(): void {
    if (!this.isGameOver) {
      ...
      if (this.uiManager.score >= this.uiManager.maxScore) {
        ...
        delay = 3000;
        this.audioManager.playAudio("endWin");
      } else {
        console.log("HAI PERSO LA PARTITA!!!");
        delay = 1000;
        this.audioManager.playAudio("endFailed");
      }

      this.audioManager.stopAudio("jeepSound");
