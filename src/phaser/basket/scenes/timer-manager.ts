...
export class TimerManager extends Phaser.Scene {
  ...
  constructor() {
    super({key: BasketAssetConf.scene.timerManager});
  }

  create(): void {
    this.addTimer();
    this.registry.set(BasketAssetConf.registry.timer, this.timerGame);
  }
  ...
  addTimer(): void {
    ...
    const iconTimer = this.add.sprite(
      ...
      BasketAssetConf.image.iconSandClock,
    );
  ...
  updateTimer(): void {
    ...
    const leftTime = +this.registry.get(BasketAssetConf.registry.timer) + 1;

    this.registry.set(BasketAssetConf.registry.timer, leftTime);
    if (this.timerText) {
  ...
  resetTimer(): void {
    this.timer = this.timerGame;
    this.timerTemp = 0;
    this.registry.set(BasketAssetConf.registry.timer, this.timer);
