...
export class Game extends Phaser.Scene {
  ...
  exitButton: undefined | Phaser.GameObjects.Image;
  ...
  //* attivato qui e disattivato in UIManager.
  createBG() {
  ...
  private initializeUIManager() {
    // Aggiungi la scena al scene manager se non è già presente
    if (!this.scene.manager.getScene(BasketAssetConf.scene.UIManager)) {
      this.scene.manager.add(BasketAssetConf.scene.UIManager, UIManager, false);
    }
 
    // Avvia la scena
    this.scene.launch(BasketAssetConf.scene.UIManager, {
      gameScene: this,
    });
 
    // Ottieni riferimento al manager
    this.uiManager = this.scene.get(BasketAssetConf.scene.UIManager) as UIManager;
