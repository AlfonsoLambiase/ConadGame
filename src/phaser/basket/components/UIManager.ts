...
export class UIManager {
  ...
  createUI(): void {
    ...
    const bgLogo = this.scene.add.image(
      this.scene.scale.width / 2,
      0,
      BasketAssetConf.image.backgroundLogo,
    );
    ...
    // logo
    const logo = this.scene.add.image(this.scene.scale.width / 2, 0, BasketAssetConf.image.logo);
    ...
    const iconScore = this.scene.add.image(
      ...
      BasketAssetConf.image.iconScore,
    );
    ...
    const backgroundScore = this.scene.add.image(
      ...
      this.gameScene.setDynamicValueBasedOnScale(-75, 110) + this.ofssetY,
      BasketAssetConf.image.backgroundScore,
    );
    ...
    this.scene.time.addEvent({
      ...
      callback: () => {
        ...
        if (steps >= repeatCount) {
          ...
          this.scene.registry.set(BasketAssetConf.registry.score, finalScore);
